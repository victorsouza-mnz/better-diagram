 ...
 *
 * CAMPO NOVO NEM SEMPRE SOBE `SCHEMA_VERSION`. `EdgeDoc.dashed`/`.bidirectional`,
 * `ContentDoc.align`/`.format` (na variante `text`) e `ContentDoc.style` (na
 * variante `shape`) são exemplo: têm um valor padrão seguro (`false`/`false`;
 * centro/meio; texto simples; preenchida — o visual de sempre), e um documento v1
 * salvo ANTES de eles existirem continua abrindo — o codec preenche o padrão na
 * leitura, mesma técnica já usada em `label`/`z` de `NodeDoc`.
...
export type ContentDoc =
  ...
  | { readonly kind: "shape"; readonly shape: string; readonly style: string }
